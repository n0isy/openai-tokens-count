...
const tokens: Record<string, number> = JSON.parse(fs.readFileSync(tokensJsonPath, 'utf-8'));

describe.each(Object.entries(tokens))('Testing %s', (file, expectedTokens) => {
  const casePath = path.join(__dirname, 'cases', file);
  const testCase: OpenAI.Chat.ChatCompletionCreateParamsNonStreaming = require(casePath).default;
