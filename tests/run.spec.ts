...
import { estimateTokens, getCachedEncoding, messageTokenCache, toolTokenCache, imageTokenCache } from "../src";
...
describe.each(Object.entries(tokens))("Testing %s", (file, expectedTokens) => {
  ...
  test(`Expected: ${expectedTokens}`, async () => {
    ...
    console.log(`Run: ${times.map( el => el.toFixed(3)).join(" ")} ms \ Average time: ${averageTime.toFixed(3)} ms. \ Messages in cache: ${messageTokenCache.size} \ ${toolTokenCache.size} \ ${imageTokenCache.size}`);
    if (expectedTokens) {
